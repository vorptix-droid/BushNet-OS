...
// src/data/deploymentFiles.ts
var import_jszip = __toESM(require("jszip"), 1);
var USERNAME = "lachlan";
var THUMBDRIVE_NAME = "THUMBDRIVE";
var PYTHON_CORE_SCRIPT = `#!/usr/bin/env python3
"""
=============================================================================
...
if __name__ == "__main__":
    main()
`;
var CLI_ASK_SCRIPT = `#!/usr/bin/env python3
"""
=============================================================================
...
def get_live_weather_telemetry():
    ...
    return {
        ...
        "rain_prob": base_rain,
        "summary": f"{temp:.1f}\xB0C | {press:.1f} hPa ({trend}, 3h \u0394: {dp_3h:+.1f}hPa) | {hum:.0f}% RH"
    }
...
def build_context_envelope(weather, query):
    ...
    probe_str = f"{probe_val:.1f}\xB0C (Active 1-Wire)" if probe_val is not None else "Standby (1-Wire)"
    ...
    summary_badge = (
        ...
        f"DHT11: {weather['temp']:.1f}\xB0C, {weather['humidity']:.0f}% RH | "
...
\u2022 LIVE SENSOR READINGS AT THIS INSTANT ({time_str}, {time_period}):
  ...
  - Ambient Air Temp: {weather['temp']:.1f}\xB0C
  - Relative Humidity: {weather['humidity']:.0f}% RH (Dew Point: {weather['temp'] - ((100 - weather['humidity']) / 5):.1f}\xB0C)
  - Waterproof Probe: {probe_str}
  - Altimeter Elevation: {weather['alt']:.0f} m
  - Rain Probability: {weather['rain_prob']}%
  - GPS Status: {gps_sats} Sats ({gps_fix}) | Lat: {gps_lat:.4f}\xB0, Lon: {gps_lon:.4f}\xB0
\u2022 CAMP & FIELD STATUS:
...
def answer_outdoors_offline(q, context):
    ...
    resp.append(f"\u{1F332} [ASPEN OUTDOORS ADVISOR] (Live Telemetry: {weather['summary']})")
    resp.append("="*65)

    if any(w in low for w in ["noise", "sound", "hear", "scary", "scared", "afraid", "panic", "dark", "creepy", "outside"]):
        resp.append("\u{1F332} WILDERNESS ACOUSTICS & NIGHT ANXIETY PROTOCOL:")
        ...
        if weather["rain_prob"] >= 50 or weather["dp_3h"] < 0:
            ...
            resp.append(" \u2022 Windbreak: Orient open face 90\xB0 to wind to draw smoke away without catching gusts.")

    elif any(w in low for w in ["fire", "wood", "kindling", "tinder", "ferro", "spark", "match"]):
        resp.append("\u{1F525} FIRE CRAFT & IGNITION GUIDANCE:")
        ...
        resp.append("\u{1F4A7} WATER SOURCING & PURIFICATION:")
        ...
        resp.append("\u{1F97E} TRAVEL & MOVEMENT ASSESSMENT:")
        ...
        resp.append(f" \u2022 Layering: Current temp {weather['temp']:.1f}\xB0C. Wear breathable synthetic/merino base layer.")
        ...
        resp.append("\u{1F43B} BACKCOUNTRY BEAR HANG (FOOD SUSPENSION) PROTOCOL:")
        ...
        resp.append("\u{1F43B} WILDLIFE & PREDATOR PROTOCOL:")
        ...
        resp.append("\u{1FA7A} FIRST AID & THERMAL REGULATION:")
        ...
        resp.append("\u{1F9ED} GENERAL FIELD STRATEGY:")
        resp.append(f" \u2022 Microclimate: Pressure {weather['pressure']:.1f} hPa ({weather['trend']}), Temp {weather['temp']:.1f}\xB0C.")
    ...
    try:
        with urllib.request.urlopen(req, timeout=12) as response:
            if show_context:
                ...
                sys.stdout.write(f"\\n\u{1F4E1} [PRE-FLIGHT CONTEXT INJECTED]: {context['summary_badge']}\\n")
...
def cmd_weather():
    ...
    print("\\n" + "="*65)
    print(" \u{1F324}\uFE0F BUSHNET ASPEN - TACTICAL METEOROLOGICAL TELEMETRY")
    print("="*65)
    print(f" \u{1F321}\uFE0F Ambient Temperature:    {weather['temp']:.1f}\xB0C")
    probe_t = read_probe_temp()
    if probe_t is not None:
        print(f" \u{1F9EA} Auxiliary Probe (1-Wire):{probe_t:.1f}\xB0C")
    print(f" \u{1F9ED} Barometric Pressure:    {weather['pressure']:.1f} hPa (3h \u0394: {weather['dp_3h']:+.2f} hPa)")
    print(f" \u{1F4C8} Barometric Tendency:    {weather['trend']}")
    print(f" \u{1F4A7} Relative Humidity:      {weather['humidity']:.0f}% RH")
    print(f" \u{1F32B}\uFE0F Calculated Dew Point:   {dew_pt:.1f}\xB0C (Dew Spread: {weather['dew_spread']:.1f}\xB0C)")
    print(f" \u26F0\uFE0F Altimeter Elevation:    {weather['alt']:.0f} meters (Calibrated from 1013.25 hPa)")
    print(f" \u{1F327}\uFE0F Rain Probability:       {weather['rain_prob']}% (Zambretti pressure tendency)")
    print("="*65)
    print(" \u{1F4A1} Commands: Type '/learn' to record a snapshot & train patterns,")
...
def cmd_learn_and_record():
    ...
    print("\\n" + "="*65)
    print(" \u{1F9E0} [ASPEN WEATHER PATTERN LEARNING & RECORDING ENGINE]")
    print("="*65)
    ...
    print(f"   - Ambient Temperature: {cur_t:.1f}\xB0C")
    print(f"   - Relative Humidity:   {cur_h:.0f}% RH")
    print(f"   - Calculated Dew Point:{dew_pt:.1f}\xB0C (Dew Spread: {dew_spread:.1f}\xB0C)")
    print(f"   - Elevation:           {weather['alt']:.0f}m")
    print()
    print(" \u{1F4C8} LEARNED ATMOSPHERIC PATTERNS & STATISTICAL TRENDS:")
    ...
    print(f" \u2022 Thermal Diurnal Range: Min {min_t:.1f}\xB0C / Max {max_t:.1f}\xB0C (\u0394: {max_t - min_t:.1f}\xB0C)")
    ...
    print()
    print(" \u{1F4BE} MODEL PARAMETERS UPDATED:")
...
def cmd_history():
    ...
    print("\\n" + "="*65)
    print(" \u{1F4CA} BUSHNET ASPEN - RECORDED WEATHER TIME-SERIES HISTORY")
    print("="*65)
    ...
    print(f" \u2022 Temperature Range:    Min {min(all_temps):.1f}\xB0C | Max {max(all_temps):.1f}\xB0C")
    ...
    for r in history[-8:]:
        ...
        t = f"{r.get('temp_c', 0):.1f}\xB0C"
        p = f"{r.get('pressure_hpa', 0):.1f}"
        h = f"{r.get('humidity', 0):.0f}%"
        d = f"{r.get('dew_point', 0):.1f}\xB0C"
        print(f" {ts:<19} | {t:<7} | {p:<11} | {h:<7} | {d:<6}")
    print("="*65)
    print(" \u{1F4A1} Tip: Type '/graph' to view visual ASCII atmospheric trend curves.")
    print("="*65 + "\\n")

def cmd_graph():
    history = load_json(HISTORY_FILE, [])
    if len(history) < 2:
        print("\\n\u{1F4C8} [GRAPH ENGINE] Need at least 2 recorded sensor points to plot trend curves.")
    ...
    print("\\n" + "="*65)
    print(" \u{1F4C8} BUSHNET ASPEN - TACTICAL ATMOSPHERIC TELEMETRY GRAPH")
    print("="*65)
    print(f" \u{1F9ED} BAROMETRIC PRESSURE TREND ({len(points)} samples, Min: {min_p:.1f} | Max: {max_p:.1f} hPa):")
    for row in rows:
        print(f" {row}")
    ...
    print(f" \u2022 Temperature Trend: [ {t_spark} ] {temp_vals[-1]:.1f}\xB0C (\u0394: {temp_vals[-1] - temp_vals[0]:+.1f}\xB0C)")
...
def cmd_fish():
    ...
    if "rapidly falling" in trend.lower() or "storm" in trend.lower():
        score = 95
        window = "\u{1F525} MAXIMUM (Pre-Frontal Feeding Binge)"
        ...
        cond = "\u{1F4C9} FALLING BAROMETER"
        ...
        cond = "\u{1F4C8} RISING BAROMETER (Post-Front High)"
        ...
        score = 30
        window = "MINIMAL"
        cond = "\u{1F327}\uFE0F LOW PRESSURE TROUGH (<1006 hPa)"
        tactical = "Storm ceiling active. Fish holding deep. Focus on passive bottom trotlines or bait resting on substrate."

    print("\\n" + "="*65)
    print(" \u{1F3A3} BUSHNET ASPEN - TACTICAL FISHING & HYDRO-METEOROLOGICAL INDEX")
    print("="*65)
    print(f" \u{1F9ED} LIVE SENSOR TELEMETRY:")
    print(f" \u2022 Barometer:            {press:.1f} hPa ({trend})")
    print(f" \u2022 Air Temp:             {temp:.1f}\xB0C | Est. Water Temp: {water_est:.1f}\xB0C")
    print(f" \u2022 Strike Activity:      {score}% [{window}]")
    print(f" \u2022 Hydro Condition:      {cond}")
    print()
    print(f" \u{1F3AF} TACTICAL FEEDING ADVICE:")
    print(f" \u2022 {tactical}")
    print()
    print(" \u{1F41F} SPECIES OPTIMAL WATER TEMPERATURES & FEEDING WINDOWS:")
    print(" \u2022 Rainbow / Cutthroat Trout (10-16\xB0C / 50-60\xB0F):")
    ...
    print()
    print(" \u{1FAA4} EMERGENCY SURVIVAL PROCUREMENT:")
...
def cmd_wildlife():
    print("\\n" + "="*65)
    print(" \u{1F43E} BUSHNET ASPEN - REGIONAL WILDLIFE & FAUNA ENCYCLOPEDIA (PNW / BOREAL)")
    print("="*65)
    print(" \u26A0\uFE0F DANGEROUS PREDATOR DEFENSE PROTOCOLS:")
    print(" \u2022 \u{1F43B} Grizzly Bear (Ursus arctos) [DANGER: CRITICAL]:")
    ...
    print(" \u2022 \u{1F43B} Black Bear (Ursus americanus) [DANGER: HIGH]:")
    ...
    print(" \u2022 \u{1F981} Cougar / Mountain Lion (Puma concolor) [DANGER: CRITICAL]:")
    ...
    print(" \u2022 \u{1F43A} Gray Wolf (Canis lupus) [DANGER: MODERATE/HIGH]:")
    print("   - Defense: Pack canines. Stand tall in group, maintain eye contact, back to campfire.")
    print(" \u2022 \u{1F98C} Moose (Alces alces) [DANGER: EXTREME DURING RUT/CALVING]:")
    print("   - Defense: 400-700kg. RUN and put large trees or boulders between you immediately!")
    print()
    print(" \u{1F357} HIGH-CALORIE EMERGENCY SURVIVAL GAME:")
    print(" \u2022 \u{1F407} Snowshoe Hare (800 kcal): 20-ga wire snares on active runs (fist loop 4 fingers up).")
    print(" \u2022 \u{1F994} Porcupine (3,000+ kcal): Slow moving; strike snout with stout staff. Roast to burn quills.")
    print(" \u2022 \u{1F332} Spruce / Ruffed Grouse (450 kcal): Low branches; take with 2m snare pole or throwing stick.")
    print(" \u2022 \u{1F9AB} Beaver (6,000+ kcal): Ponds/dams; tail is rich concentrated survival fat.")
    print()
    print(" \u{1F40D} VENOMOUS & NUISANCE FAUNA:")
    print(" \u2022 \u{1F40D} Western Rattlesnake (arid rocky slopes): Freeze if buzzing, back away 3m.")
    print(" \u2022 \u{1F577}\uFE0F Ticks: Daily check (groin/armpits); remove with steady tweezer pull.")
    print("="*65 + "\\n")
...
def cmd_status():
    ...
    print("\\n" + "="*65)
    print(" \u{1F6E0}\uFE0F BUSHNET ASPEN - HARDWARE & SUBSYSTEM STATUS")
    print("="*65)
    print(f" \u2022 BMP180 Barometer:    ONLINE ({weather['pressure']:.1f} hPa, {weather['alt']:.0f}m)")
    print(f" \u2022 DHT11 Sensor:        ONLINE ({weather['temp']:.1f}\xB0C, {weather['humidity']:.0f}% RH)")
    probe_str = f"ONLINE ({probe_temp:.1f}\xB0C)" if probe_temp is not None else "STANDBY (1-Wire)"
...
def cmd_help():
    print("\\n" + "="*65)
    print(" \u{1F332} BUSHNET ASPEN - COMMAND SYSTEM REFERENCE")
    print("="*65)
    print(" COMMANDS:")
...
def process_query(query, model="qwen2.5:1.5b", show_context=False):
    ...
    if q in ["/poweroff", "/shutdown"] or any(k in low for k in ["poweroff", "power off", "shutdown", "turn off"]):
        print("\u{1F6D1} [SHUTDOWN] Syncing files and powering off Pi 5 safely...")
...
def main():
    ...
    print("=" * 65)
    print(f" \u{1F332} BushNet ASPEN - Survival AI Shell (Operator: {OPERATOR})")
    print(f" \u{1F4E1} Pre-Flight Telemetry: {context['summary_badge']}")
    print(f" \u{1F9E0} Active Model: {model} (auto-fallback enabled)")
...
if __name__ == "__main__":
    main()
`;
var INGEST_MANUALS_SCRIPT = `#!/usr/bin/env python3
"""
=============================================================================
...
def init_seed_database():
    ...
    for seed in SEED_MANUALS:
        if seed["title"] not in existing_titles:
            ...
            added += 1
    save_index(existing)
    print(f"\u{1F332} Initialized offline survival knowledge base with {len(existing)} verified manual chapters ({added} added).")

def download_core_manuals():
    """Downloads core public-domain military and wilderness survival PDFs into manuals directory."""
    ensure_dirs()
    print("=" * 65)
    print(" \u{1F4E5} DOWNLOADING CORE SURVIVAL MANUALS (TruePrepper & US Army)")
    print("=" * 65)
...
def index_all_manuals():
    ...
    if not pdf_files:
        print(f"\\n\u{1F4C2} No custom manual files found in {MANUALS_DIR}.")
    ...
    print(f"\\n\u{1F50D} Found {len(pdf_files)} manual file(s) in {MANUALS_DIR}. Indexing...")
    total_added = 0
    ...
    save_index(existing)
    print(f"\u{1F389} Index complete! Total searchable knowledge snippets: {len(existing)} ({total_added} new from local files).\\n")
...
def show_status():
    ...
    print("=" * 65)
    print(" \u{1F4DA} BUSHNET ASPEN - OFFLINE MANUAL & RAG STATUS")
    print("=" * 65)
...
`;
var INSTALL_BASH_SCRIPT = `#!/usr/bin/env bash
# =============================================================================
...
echo "=========================================================="
`;
var THUMBDRIVE_README = `# \u{1F332} BushNet ASPEN - USB Thumb Drive Setup & Operation Manual
...
## \u{1F916} Asking the Local Survival AI (Qwen 0.5B & 1.5B)
...
## \u{1F4DA} Survival PDF & Field Manual Ingestion (Offline RAG)
...
`;
var START_KIOSK_SCRIPT = `#!/bin/bash
# =====================================================================
...
`;
var KIOSK_SYSTEMD_SERVICE = `[Unit]
...
`;
var SETUP_PI5_QWEN_SCRIPT = `#!/bin/bash
# =====================================================================
...
echo "=== \u{1F332} BushNet Pi 5 Offline AI Setup ==="
...
`;
var WIRING_GUIDE_35_HUB = `# BushNet 3.5" Handheld Cyberdeck & Arduino Sensor Hub Guide

## 1. Hardware Overview
- **Core Computer**: Raspberry Pi 5 (2GB or 4GB RAM)
- **Display**: Hosyoond 3.5" TFT Touchscreen (480\xD7320)
...
app.post("/api/aspen/chat", async (req, res) => {
  try {
    ...
    const isTempDrop = sensors.dhtTemp !== void 0 && sensors.dhtTemp < 15;
    ...
    const isComplex = activeEmergency || isColdFront || isStormDrop || /medical|symptom|treat|doctor|first aid|allergic|asthma|diabetes|injury|infection|poison|fracture|cpr|hypothermia|shelter build|calculate ration|storm|pressure|cold front|hpa|weather/i.test(prompt) || prompt.length > 120;
    if (modelPreference === "auto") {
      if (isComplex || activeEmergency) {
        selectedModel = "qwen-1.5b";
        autoReasoning = isColdFront ? "Routed to Qwen 2.5 1.5B (Detected High-Pressure Cold Front Surge: Rapid hPa rise + Temp drop)" : isStormDrop ? "Routed to Qwen 2.5 1.5B (Detected Storm Front: Rapid Barometric Pressure Drop)" : "Routed to Qwen 2.5 1.5B (Requires complex medical/survival reasoning or emergency calculation)";
      } else {
    ...
    const ragDatabase = [
      {
        ...
        snippet: "FM 3-05.70 Mandate: Tinder must be bone-dry. Birch bark shaves, dry cedar fiber, and punk wood yield lowest ignition threshold. Scrape ferro rod at 45\xB0 angle. Debris shelter walls require minimum 3 feet (1m) of leaves/pine needles for sub-zero thermal insulation."
      },
      {
    ...
    const matchedRagChunks = ragDatabase.filter(
      (doc) => doc.tags.some((tag) => prompt.toLowerCase().includes(tag))
    );
    const ragContext = matchedRagChunks.length > 0 ? matchedRagChunks.map((c) => `[RETRIEVED SOURCE: ${c.source}]
...
1. DO NOT assume the operator possesses specific branded or unlogged gear (e.g. "Mora knife", "hatchet", "headlamp", "Glock entrenching tool") unless explicitly listed in the logged Gear list below. Refer only to general actions or gear listed in memory.
   LOGGED OPERATOR GEAR LIST: ${gear.length > 0 ? gear.map((g) => typeof g === "string" ? g : g.name).join(", ") : "Standard survival kit"}
2. UNLOGGED ITEM DETECTION: If the operator mentions carrying, holding, or using an unlogged tool/item (e.g. "I'm holding my hatchet", "I have a Mora knife", "using my headlamp", "wearing my thermal parka") that is NOT in the logged gear list above, ASPEN MUST append this exact line at the end of the response:
   "\u{1F4A1} UNLOGGED GEAR DETECTED: I noticed you mentioned possessing [Item Name]. Would you like me to add [Item Name] to your gear inventory?"
...
\u2022 You possess deep knowledge of fish swim-bladder biology, barometric pressure dynamics, and aquatic habitat ecology:
  ...
  - Target Species & Temperature Brackets:
    * Rainbow / Cutthroat / Brook Trout: Ideal water 10-16\xB0C (50-60\xB0F). Lethargic above 20\xB0C.
    * Salmon (Chinook, Coho, Pink, Sockeye): Ideal water 8-14\xB0C (46-58\xB0F). Upstream movement during overcast/rain.
    * Smallmouth / Largemouth Bass: Ideal water 18-24\xB0C (65-75\xB0F).
    * Walleye & Northern Pike: Ideal water 12-18\xB0C (55-65\xB0F). Low-light shallows for Walleye; weed lines for Pike.
    * White / Atlantic Sturgeon: Deep river holes (8-16\xB0C), passive scavengers on substrate.
...
- Medical Conditions & Allergies: ${medical && medical.length > 0 ? medical.map((m) => `${m.condition || m} (${m.severity || "Active"})`).join(", ") : "None recorded"}
- Sensor Data: Temp ${sensors.dhtTemp ?? 21.5}\xB0C, Baro Pressure ${sensors.bmpPressure ?? 1013.25} hPa (${sensors.pressureTrend || "Stable"}), Probe Temp ${sensors.kyTemp ?? 19.8}\xB0C, GPS Satellites: ${sensors.gpsSats ?? 7}
- Camp & Inventory State: Shelters: ${shelters.length}, Food Items: ${food.length}, Water: ${sensors.waterSupplyLiters ?? 4.5}L, Gear: ${gear.map((g) => g.name || g).join(", ") || "Standard kit"}
...
\`\`\`
`;
    const preFlightSummary = `BMP180: ${sensors.bmpPressure || 1013.2} hPa (${sensors.pressureTrend || "Stable"}) | DHT11: ${sensors.dhtTemp || 21.5}\xB0C, ${sensors.dhtHum || 50}% RH | Probe: ${sensors.kyTemp || 19.8}\xB0C | GPS: ${sensors.gpsSats || 7} Sats locked | Mic/Audio: NONE`;
...
\u2022 LIVE SENSOR READINGS:
  ...
  - Ambient Air Temp: ${sensors.dhtTemp ?? 21.5}\xB0C
  - Relative Humidity: ${sensors.dhtHum ?? 50}% RH
  - Auxiliary Probe: ${sensors.kyTemp ?? 19.8}\xB0C
  - Elevation: ${sensors.bmpAlt ?? 23}m
  - GPS Position: ${sensors.gpsSats ?? 7} Satellites locked (${sensors.gpsLat ?? 49.2827}\xB0N, ${sensors.gpsLng ?? -123.1207}\xB0W, Compass: ${sensors.headingDeg ?? 0}\xB0)
...
[Status: BushNet Core running local offline guidance]` : `[ASPEN LOCAL ENGINE]: Telemetry nominal. Sensors online. Ready for command.`;
    ...
    if (lcdMatch) {
      ...
      if (activeEmergency) {
        ...
        const totalKcal = food.reduce((acc, f) => acc + (Number(f.totalCalories) || 0), 0);
        lcdLine1 = "FOOD STORAGE";
        lcdLine2 = `${totalKcal} total kcal`;
      } else {
        lcdLine1 = `ASPEN (${selectedModel === "qwen-1.5b" ? "1.5B" : "0.5B"})`;
        lcdLine2 = `T:${sensors.dhtTemp || 21}\xB0C P:${Math.round(sensors.bmpPressure || 1013)}`;
      }
    }
    res.json({
      text: fullText,
      ...
      ragMatches: matchedRagChunks.map((m) => ({ source: m.source, title: m.title, category: m.category })),
      lcdDisplay: {
